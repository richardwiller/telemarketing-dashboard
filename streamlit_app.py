...
def main():
    ...
    st.set_page_config(page_title = 'Telemarketing Analysis', \
        page_icon = './img/artboard_123063.ico',
        layout="wide",
    ...
    if (data_file_1 is not None):
        ...
        graphs1, buff, graphs2 = st.columns([2, 0.3, 2])
        # PLOTS    
        if graph_type == 'Barras':
            with graphs1:
                chart1 = alt.Chart(bank_raw_target_perc.reset_index(drop=False)).mark_bar().encode(
                    x='y',
                    y='proportion',
                    color ='proportion'
                    ).properties(
                ...
                st.write(chart1)
            with graphs2:
                chart2 = alt.Chart(bank_target_perc.reset_index(drop=False)).mark_bar().encode(
                    x='y',
                    y='proportion',
                    color ='proportion'
                    ).properties(
            ...
            with graphs1:
                chart3 = alt.Chart(bank_raw_target_perc.reset_index(drop=False)).mark_arc().encode(
                    theta = 'proportion',
                    color = 'y'
                    ).properties(
                        width=600,
                ...
                st.write(chart3)
            with graphs2:
                chart4 = alt.Chart(bank_target_perc.reset_index(drop=False)).mark_arc().encode(
                    theta = 'proportion',
                    color = 'y'
                    ).properties(
                        width=600,
